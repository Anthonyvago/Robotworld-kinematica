 ...
 * @param aStartpoint
 * @param anEndPoint
 * @return the counterclockwise angle in radians between the positive X axis and
 ...
 */
/* static */ double
Shape2DUtils::getAngle(const Model::BoundedVector &aVector) {
  return GetAngle(Point(0, 0), Point(static_cast<int>(aVector.x),
                                     static_cast<int>(aVector.y)));
}
/**
 *
 */
...
/* static */ bool Shape2DUtils::intersect(const Point &aStartLine1,
  ...
  return getIntersection(aStartLine1, aEndLine1, aStartLine2, anEndLine2) !=
         DefaultPosition;
}
/**
 *
...
/* static */ bool Shape2DUtils::isOnLine(const Point &aStartPoint,
  ...
  double normalLength = std::sqrt(
      (aStartPoint.x - anEndPoint.x) * (aStartPoint.x - anEndPoint.x) +
      (aStartPoint.y - anEndPoint.y) * (aStartPoint.y - anEndPoint.y));

  double distance =
      std::abs((aPoint.x - anEndPoint.x) * (aStartPoint.y - anEndPoint.y) -
  ...
  //	long long c = (x3 * x3) + (y3 * y3) + (x1 * x1) + (y1 * y1) - (2 * ((x3
  //* x1) + (y3 * y1))) - (r * r); 	long long i = (b * b) - (4 * a * c);
  //
  //	if (i < 0)
  //	{
  // No intersection
 ...
 *
 */
/* static */ Shape2DUtils::CompassPoint
Shape2DUtils::getCompassPoint(const Point &aPoint, const Size &aSize,
                              const Point &aBorderPoint, int aRadius /*= 3*/) {
  Point centre = aPoint;
  Size size = aSize;
  Point northWest(centre.x - (size.x / 2),
                  centre.y - (size.y / 2)); // upper left
  Point northEast(centre.x + (size.x / 2),
                  centre.y - (size.y / 2)); // upper right
  Point southEast(centre.x + (size.x / 2),
                  centre.y + (size.y / 2)); // right under
  Point southWest(centre.x - (size.x / 2),
                  centre.y + (size.y / 2)); // left under
  ...
  if (aBorderPoint == northWest) {
    ...
    return CP_SOUTHWEST;
  } else if (Shape2DUtils::isOnLine(northWest, northEast, aBorderPoint,
                                    aRadius)) {
    return CP_NORTH;
  } else if (Shape2DUtils::isOnLine(northEast, southEast, aBorderPoint,
                                    aRadius)) {
    return CP_EAST;
  } else if (Shape2DUtils::isOnLine(southEast, southWest, aBorderPoint,
                                    aRadius)) {
    return CP_SOUTH;
  } else if (Shape2DUtils::isOnLine(southWest, northWest, aBorderPoint,
                                    aRadius)) {
    return CP_WEST;
  }
...
/* static */ bool Shape2DUtils::isCompassPoint(const Point &aPoint,
                                               ...
                                               int aRadius /* = 6 */) {
  bool result =
      getCompassPoint(aPoint, aSize, aBorderPoint, aRadius) == aCompassPoint;
  if (result == true) {
    return result;
  }
  return result;
  // return getCompassPoint( aPoint, aSize, aBorderPoint,aRadius) ==
  // aCompassPoint;
}
/**
 *
