...
RectangleShape::RectangleShape(const std::string &aTitle /*= ""*/)
    : centre(-1, -1), size(0, 0), title(aTitle), titleSize(0, 0),
...
RectangleShape::RectangleShape(Model::ModelObjectPtr aModelObject,
    ...
    : Shape(aModelObject), centre(aCentrePoint), size(0, 0), title(aTitle),
      titleSize(0, 0), borderWidth(aBorderWidth), spacing(aSpacing) {}
/**
 *
 */
RectangleShape::RectangleShape(const Point &aCentrePoint,
                               const std::string &aTitle /*= ""*/,
                               int aBorderWidth /*= 2*/, int aSpacing /*= 2*/)
    : centre(aCentrePoint), size(0, 0), title(aTitle), titleSize(0, 0),
...
void RectangleShape::draw(wxDC &dc) {
  ...
  if (isSelected()) {
    dc.SetPen(
        wxPen(WXSTRING(getSelectionColour()), borderWidth, wxPENSTYLE_SOLID));
  } else {
    dc.SetPen(
        wxPen(WXSTRING(getNormalColour()), borderWidth, wxPENSTYLE_SOLID));
  }
  ...
  dc.DrawText(WXSTRING(title), centre.x - titleSize.x / 2,
              y + spacing + borderWidth);
}
/**
 *
 */
bool RectangleShape::occupies(const Point &aPoint) const {
  if (centre.x - (size.x / 2) <= aPoint.x &&
      aPoint.x <= centre.x + (size.x / 2) &&
      centre.y - (size.y / 2) <= aPoint.y &&
      aPoint.y <= centre.y + (size.y / 2)) {
    return true;
  }
  return false;
}
...
bool RectangleShape::isBorderPoint(const Point aPoint,
  ...
  return Utils::Shape2DUtils::isOnLine(rectanglePoints, 4, aPoint,
                                       aRadius + borderWidth);
}
/**
 *
 */
Point RectangleShape::getCentre() const { return centre; }
/**
 *
 */
void RectangleShape::setCentre(const Point &aPoint) { centre = aPoint; }
/**
 *
 */
std::string RectangleShape::getTitle() const { return title; }
/**
 *
 */
void RectangleShape::setTitle(const std::string &aTitle) { title = aTitle; }
/**
 *
 */
Size RectangleShape::getSize() const { return size; }
/**
 *
 */
 ...
 */
// cppcheck-suppress unusedFunction
int RectangleShape::getBorderWidth() const { return borderWidth; }
/**
 *
 */
// cppcheck-suppress unusedFunction
 ...
 */
// cppcheck-suppress unusedFunction
int RectangleShape::getSpacing() const { return spacing; }
/**
 *
 */
// cppcheck-suppress unusedFunction
void RectangleShape::setSpacing(int aSpacing) { spacing = aSpacing; }
...
std::string RectangleShape::asString() const {
  ...
  os << "spacing: " << spacing << ", ";
  os << "modelObject: "
     << (getModelObject() ? getModelObject()->asString() : "none");
...
std::string RectangleShape::asDebugString() const {
  ...
  os << "\tspacing: " << spacing << "\n";
  os << "\tmodelObject: "
     << (getModelObject() ? getModelObject()->asDebugString() : "none");
