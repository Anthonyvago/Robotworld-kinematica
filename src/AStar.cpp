...
Path AStar::search(Vertex aStart, const Vertex &aGoal, const Size &aRobotSize) {
  ...
  int radius = static_cast<int>(
      std::ceil(std::sqrt((aRobotSize.x / 2.0) * (aRobotSize.x / 2.0) +
  ...
  aStart.heuristicCost =
      ...
      HeuristicCost(
          aStart,
          aGoal); // Estimated total cost from aStart to aGoal through y.
  ...
  while (!openSet.empty()) {
    ...
    if (current.equalPoint(aGoal)) {
      ...
      //				std::cout << "Duration: " <<
      // static_cast<double>(end - start)/CLOCKS_PER_SEC <<	", openSet: " <<
      // openSet.size() <<  ", closedSet: " << closedSet.size() << ",
      // predecessorMap: " << predecessorMap.size() << std::endl;
      ...
      const std::vector<Edge> &connections =
          GetNeighbourConnections(current, radius);
      ...
      for (const Edge &connection : connections) {
        ...
        neighbour.heuristicCost =
            neighbour.actualCost + HeuristicCost(neighbour, aGoal);

        // The neighbour may already be in the openSet because of the previous
        // current Vertex iteration
        OpenSet::iterator openVertex = findInOpenSet(neighbour);
        if (openVertex != openSet.end()) {
          // if neighbour is in the openSet we may have found a shorter
          // via-route than via the previous current Vertex
          if ((*openVertex).heuristicCost <= neighbour.heuristicCost) {
            // Do nothing
            continue;
          } else {
            // Update the cost
            // TODO: should the actual cost be adjusted if
        ...
        if (closedVertex != closedSet.end()) {
          // if neighbour is in the closedSet we may have found a shorter
          // via-route
        ...
        const auto &[iterator, succes] =
            predecessorMap.insert_or_assign(neighbour, current);
        if (!succes) {
          std::cerr << "**** Failed adding neighbour to the predecessorMap: "
                    << neighbour << std::endl;
        }
      ...
      //			Duration: 17.349.490 openSet: 1507 closedSet:
      // 76521 predecessorMap: 78027
      //
      //			After the insert of all:
      //			aRobotSize = (37,29), radius = 23
      //			Duration: 7.733.159 openSet: 1507 closedSet:
      // 76521 predecessorMap: 78027
      //
      //			Partial sort after insert of all:
      //			aRobotSize = (37,29), radius = 23
      //			Duration: 3.297.669 openSet: 1507 closedSet:
      // 76521 predecessorMap: 78027
      //
      //			No sorting, just iterator swap for begin() and
      // the minimum element, which is basically 			the
      // smallest partial_sort there
      // is... 			aRobotSize = (37,29), radius = 23
      // Duration: 2.626.220 openSet: 1507 closedSet: 76521 predecessorMap: 78027
      //
      //			// Sort the openSet before the next loop, use
      // partial_sort as per Scott Meyers' Effective STL
      //			if(std::distance(openSet.begin(), openSet.end())
      //> 5 )
      //			{
      //				std::partial_sort( openSet.begin(),
      // openSet.begin() + 4, openSet.end(), VertexLessCostCompare());
      // }else
      //			{
      //				std::sort( openSet.begin(),
      // openSet.end(), VertexLessCostCompare());
      //			}
      //
      //			 28-1-2022:
      //
      //			 Some rationalisations (see a diff for the
      // differences) and the size of the window has changed.
      // Don't know if that should have changed the number of nodes though.
      // Timings are without enabling notyfyObservers but with checking if the
      // are enabled. Removing
      // the check 			 did not change the times significantly.
      // The numbers have changed but the essence stays the same: using a vector
      // and searching/swapping 			 is faster than using a (always
      // sorted) set: 			   With profiling information:
      // Duration: 0.487936, openSet: 1252, closedSet: 83731, predecessorMap:
      // 84982 			   Without profiling information:
      // Duration: 0.294032, openSet: 1252, closedSet: 83731, predecessorMap:
      // 84982
 ...
 *
 */
void AStar::removeFirstFromOpenSet() { openSet.erase(openSet.begin()); }
/**
 *
 */
 ...
 */
// cppcheck-suppress unusedFunction
ClosedSet AStar::getClosedSet() const { return closedSet; }
/**
 *
 */
// cppcheck-suppress unusedFunction
 ...
 *
 */
OpenSet AStar::getOpenSet() const { return openSet; }
/**
 *
 */
// cppcheck-suppress unusedFunction
VertexMap AStar::getPredecessorMap() const { return predecessorMap; }

/**
 *
 */
ClosedSet &AStar::getCS() { return closedSet; }
/**
 *
 */
const ClosedSet &AStar::getCS() const { return closedSet; }
/**
 *
 */
OpenSet &AStar::getOS() { return openSet; }
/**
 *
 */
const OpenSet &AStar::getOS() const { return openSet; }
/**
 *
 */
VertexMap &AStar::getPM() { return predecessorMap; }
/**
 *
 */
const VertexMap &AStar::getPM() const { return predecessorMap; }
} // namespace PathAlgorithm
