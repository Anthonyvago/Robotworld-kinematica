...
void AbstractSensor::setOn(unsigned long aSleepTime /*= 100*/) {
  ...
  if (running == false) {
    running = true;
    std::thread newSensorThread([this, aSleepTime] { run(aSleepTime); });
 ...
 *
 */
void AbstractSensor::sendPercept(
    std::shared_ptr<AbstractPercept> anAbstractPercept) {
...
void AbstractSensor::run(unsigned long aSleepTime) {
  try {
    while (running == true) {
      ...
      std::shared_ptr<AbstractPercept> currentPercept =
          getPerceptFor(currentStimulus);
 ...
 *
 */
void AbstractSensor::attachAgent(AbstractAgent *anAgent) { agent = anAgent; }
/**
 *
 */
void AbstractSensor::detachAgent() { agent = nullptr; }
/**
 *
 */
std::string AbstractSensor::asString() const { return "AbstractSensor"; }
/**
 *
 */
std::string AbstractSensor::asDebugString() const { return asString(); }
} // namespace Model
