...
std::shared_ptr<AbstractStimulus> LaserDistanceSensor::getStimulus() const {
  ...
  for (std::shared_ptr<Wall> wall : walls) {
    ...
    if (robot) {
      ...
      double angle =
          Utils::Shape2DUtils::getAngle(robot->getFront()) + 0.5 * Utils::PI;

      Point laserEndpoint{
          static_cast<int>(robotLocation.x +
                           std::cos(angle - 0.5 * Utils::PI) * 1024),
          static_cast<int>(robotLocation.y +
                           std::sin(angle - 0.5 * Utils::PI) * 1024)};

      Point interSection = Utils::Shape2DUtils::getIntersection(
      ...
      if (interSection != DefaultPosition) {
        ...
        return std::shared_ptr<AbstractStimulus>(
            new DistanceStimulus(angle, distance));
      }
    }
  }

  std::shared_ptr<AbstractStimulus> distanceStimulus(
      new DistanceStimulus(666, 666));
  return distanceStimulus;
}
/**
 *
 */
std::shared_ptr<AbstractPercept> LaserDistanceSensor::getPerceptFor(
    std::shared_ptr<AbstractStimulus> anAbstractStimulus) {
  auto *distanceStimulus =
      dynamic_cast<DistanceStimulus *>(anAbstractStimulus.get());
  if (distanceStimulus) {
    return AbstractPerceptPtr(new DistancePercept(distanceStimulus->angle,
 ...
 *
 */
std::string LaserDistanceSensor::asDebugString() const { return asString(); }
} // namespace Model
