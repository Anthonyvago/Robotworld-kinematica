...
void CommunicationService::registerServer(ServerPtr aServer,
                                          bool start /* = true */) {
  // TODO Should this be an assert during development only in the limited
  // context of this example?
  auto result = servers.find(aServer->getPort());
  if (result != servers.end()) {
    std::ostringstream os;
    os << __PRETTY_FUNCTION__
       << ": only one server per port allowed, port = " << aServer->getPort();
...
void CommunicationService::stopServer(unsigned short aPort,
  ...
  if (result != servers.end()) {
    ...
    if (servers.empty()) {
      // The number of seconds needs to be larger than the number of seconds it
      // takes the server to stop handling requests (which is 1 second now)??
      timer.expires_from_now(boost::posix_time::seconds(2));
      timer.async_wait([this](const boost::system::error_code &UNUSEDPARAM(
                           e)) // @suppress("Method cannot be resolved")
                       {
                         boost::asio::defer(
                             [this]() // @suppress("Invalid arguments")
                             { this->stop(); });
                       });
    }
  } else {
 ...
 *
 */
void CommunicationService::stop() { io_context.stop(); }
/**
 *
 */
bool CommunicationService::isStopped() { return io_context.stopped(); }
/**
 *
 */
 ...
 *
 */
void CommunicationService::wait() { io_contextThread.join(); }
...
void CommunicationService::run_io_context() {
  ...
  // @see
  // https://www.boost.org/doc/libs/1_76_0/doc/html/boost_asio/reference/io_context.html
