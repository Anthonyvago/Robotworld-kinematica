 ...
 *
 */
WallShape::WallShape(Model::WallPtr aWall, RectangleShapePtr aRectangleShape1,
                     RectangleShapePtr aRectangleShape2)
    : LineShape(std::dynamic_pointer_cast<Model::ModelObject>(aWall),
                aRectangleShape1, aRectangleShape2, "", 1, 0) {}
/**
 *
 */
WallShape::~WallShape() {}
...
void WallShape::draw(wxDC &dc) {
  ...
  dc.DrawLine(getNode1()->getCentre().x, getNode1()->getCentre().y,
              getNode2()->getCentre().x, getNode2()->getCentre().y);
}
/**
 *
 */
...
std::string WallShape::asString() const {
  ...
  os << LineShape::asDebugString()
     << " WallShape: " << Utils::Shape2DUtils::asString(getBegin()) << "->"
