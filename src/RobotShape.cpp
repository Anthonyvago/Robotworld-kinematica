...
RobotShape::RobotShape(Model::RobotPtr aRobot)
    : RectangleShape(std::dynamic_pointer_cast<Model::ModelObject>(aRobot),
                     aRobot->getPosition(), aRobot->getName()),
...
void RobotShape::handleActivated() {
  ...
  if (goal) {
    ...
    getRobot()->setFront(Model::BoundedVector(goalPosition, robotPosition),
                         false);
  }
}
/**
...
void RobotShape::handleSelection() {
  //		if (robotWorldCanvas->isShapeSelected() &&
  // robotWorldCanvas->getSelectedShape()->getObjectId() == getObjectId())
  //		{
  //			setSelected();
  //		} else
  //		{
...
void RobotShape::draw(wxDC &dc) {
  ...
  Point cornerPoints[] = {getRobot()->getFrontRight(),
                          getRobot()->getFrontLeft(), getRobot()->getBackLeft(),
  ...
  double angle =
      Utils::Shape2DUtils::getAngle(getRobot()->getFront()) + 0.5 * Utils::PI;
  ...
  dc.DrawRotatedText(WXSTRING(title), centre.x - titleSize.x / 2,
                     centre.y - titleSize.y / 2, angle - Utils::PI);
}
/**
 *
 */
