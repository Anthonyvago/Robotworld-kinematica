 ...
 */
WayPointPtr
RobotWorld::newWayPoint(const std::string &aName /*= "new WayPoint"*/,
                        const Point &aPosition /*= Point(-1,-1)*/,
                        bool aNotifyObservers /*= true*/) {
 ...
 *
 */
PathPtr RobotWorld::newPath(const std::string &aName, const wxColour &pathColor,
...
void RobotWorld::deletePath(PathPtr aPath, bool aNotifyObservers) {
  auto it =
      std::find_if(paths.begin(), paths.end(),
                   [&aPath](const PathPtr &path) { return path == aPath; });
...
PathPtr RobotWorld::getPath(const Base::ObjectId &anObjectId) const {
  if (auto i = std::find_if(paths.begin(), paths.end(),
 ...
 *
 */
const std::vector<RobotPtr> &RobotWorld::getRobots() const { return robots; }
/**
 *
 */
 ...
 *
 */
const std::vector<GoalPtr> &RobotWorld::getGoals() const { return goals; }
/**
 *
 */
const std::vector<WallPtr> &RobotWorld::getWalls() const { return walls; }
/**
 *
 */
const std::vector<PathPtr> &RobotWorld::getPaths() const { return paths; }
/**
 *
 */
void RobotWorld::populate(const std::vector<eFilterType> &activeFilters,
  ...
  RobotPtr robot = newRobot("Robot", robotLocation,
  ...
  PathPtr beliefKalmanPath =
      newPath("Belief-kalman", KALMAN_BLUE, POSITIONAL_BLACK);
  PathPtr beliefParticlePath =
      newPath("Belief-particle", PARTICLE_GREEN, POSITIONAL_BLACK);
 ...
 *
 */
std::string RobotWorld::asString() const { return ModelObject::asString(); }
/**
 *
 */
