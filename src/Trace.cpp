 ...
 *
 */
/* static */ std::unique_ptr<ITraceFunction> Trace::traceFunction =
    std::make_unique<StdOutTraceFunction>();
/**
 *
 */
 ...
 *
 */
Trace::Trace(const std::string &aText, long aLineNumber /* = 0 */,
 ...
 *
 */
Trace::Trace(const std::string &aText1, const std::string &aText2,
             long aLineNumber /* = 0 */, bool withTimeStamp /* = false */,
             bool aTraceCDtor /* = true*/)
    : traceText(aText1 + " " + aText2), traceTimeStamp(withTimeStamp),
 ...
 */
/* static */ void
Trace::trace(const std::string &aText, const char *aFunctionName,
             long aLineNumber,
             bool withTimeStamp /* = Trace::isTimeStampEnabled() */) {
 ...
 *
 */
/* static */ void Trace::traceFormattedString(
    ...
    char aTraceMarker) // 	This text will be prepended
{
  if (Trace::traceOn) {
    ...
    if (Trace::traceThreadId) {
      ...
      if (threadIndentionLevels.find(currentThreadId) !=
          threadIndentionLevels.end())
#else
      if (!threadIndentionLevels.contains(
              currentThreadId)) // @suppress("Method cannot be resolved")
#endif
      {
