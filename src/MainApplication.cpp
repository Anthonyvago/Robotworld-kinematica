...
/* static */ std::vector<CommandlineArgument>
    MainApplication::commandlineArguments;
...
wxIMPLEMENT_APP_NO_MAIN(
    MainApplication); // @suppress("C-Style cast instead of C++ cast")
...
bool MainApplication::OnInit() {
  ...
  if (MainApplication::isArgGiven("-worldname")) {
    Base::ObjectId::objectIdNamespace =
        MainApplication::getArg("-worldname").value + "-";
...
/* static */ void MainApplication::setCommandlineArguments(int theArgc,
  ...
  // argv[0] contains the executable name as one types on the command line (with
  // or without extension)
  MainApplication::commandlineArguments.push_back(
      CommandlineArgument(0, "Executable", theArgv[0]));
  ...
  for (unsigned int i = 1; i < static_cast<unsigned int>(theArgc); ++i) {
    ...
    if (currentArg[0] == '-') // ordinary argument
    {
      ...
      // First handle the arguments in the form of "variable=value", and find
      // the "="
      ...
      // If given on the command line than the variable will be set to true as
      // if variable=true is passed
      if (inserted == false) {
...
/* static */ bool MainApplication::isArgGiven(const std::string &aVariable) {
  std::vector<CommandlineArgument>::iterator i =
      std::find(MainApplication::commandlineArguments.begin(),
...
MainApplication::getArg(const std::string &aVariable) {
  std::vector<CommandlineArgument>::iterator i =
      std::find(MainApplication::commandlineArguments.begin(),
                MainApplication::commandlineArguments.end(), aVariable);
 ...
 */
/* static */ CommandlineArgument &
MainApplication::getArg(unsigned long anArgumentNumber) {
...
/* static */ void MainApplication::readFileConfiguration() {
  ...
  if (Utils::FileIO::readFile(filePath + fileName, lines, fromRoot)) {
    try {
      for (const std::string &line : lines) {
        std::vector<std::string> elements =
            Utils::StringUtils::Divide(line, ';');

        std::string argName = "-" + elements.at(0);
        std::string argValue = elements.at(1);

        double valueTest =
            std::stod(argValue); // check if value is correct type.
        UNUSEDCAST(valueTest);

        CommandlineArgument argument(commandlineArguments.size(), argName,
                                     argValue);
        commandlineArguments.emplace_back(argument);
      }
    } catch (std::out_of_range &e) {
      std::cerr << __FUNCTION__
                << ": given config file has incorrect format -> " << fileName
                << std::endl;
    ...
    std::cerr << __FUNCTION__
              << "given configuration file does not exist, go to README.md for "
                 "instructions."
              << std::endl;
  }
}
