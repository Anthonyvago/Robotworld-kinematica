...
RobotWorldCanvas::RobotWorldCanvas(Window *anOwner)
    : ScrolledCanvas(anOwner, DEFAULT_ID), popupPoint(DefaultPosition),
      startActionPoint(DefaultPosition), endActionPoint(DefaultPosition),
      actionOffset(DefaultPosition), startActionSize(DefaultSize),
      startActionShape(nullptr), endActionShape(nullptr),
      selectedShape(nullptr), activationEnabled(false), selectionEnabled(false),
      menuItemEnabled(false), dandEnabled(true), notificationHandler(nullptr) {
  // CppCheck gives a "virtualCallInConstructor" on initialise(). I don't know
  // why. It cannot be suppressed by a "cppcheck-suppress
  // virtualCallInConstructor" (10-4-2022)
  initialise();
}
...
RobotWorldCanvas::RobotWorldCanvas(Window *anOwner,
    ...
    :
      ...
      ScrolledCanvas(anOwner, DEFAULT_ID), ViewObject(aModelObject),
      popupPoint(DefaultPosition), startActionPoint(DefaultPosition),
      endActionPoint(DefaultPosition), actionOffset(DefaultPosition),
      startActionSize(DefaultSize), startActionShape(nullptr),
      endActionShape(nullptr), selectedShape(nullptr), activationEnabled(false),
      selectionEnabled(false), menuItemEnabled(false), dandEnabled(true),
...
Point RobotWorldCanvas::devicePointFor(const Point &aScreenPoint) const {
  Point devicePoint;
  /*owner->getCanvas()->*/ CalcUnscrolledPosition(
      aScreenPoint.x, aScreenPoint.y, &devicePoint.x, &devicePoint.y);
  return devicePoint;
}
/**
 *
 ...
 *
 */
ShapePtr RobotWorldCanvas::getSelectedShape() const { return selectedShape; }
/**
 *
 */
...
ShapePtr RobotWorldCanvas::getShapeAt(const Point &aPoint) const {
  if (auto i = std::find_if(
          ...
          [&aPoint](ShapePtr shape) { return shape->occupies(aPoint); });
      i != shapes.end()) {
    return *i;
  }
  return nullptr;
 ...
 *
 */
void RobotWorldCanvas::handleNotification() { handleBackGroundNotification(); }
/**
 *
 */
 ...
 *
 */
void RobotWorldCanvas::populate(
    const std::vector<Model::eFilterType> &activeFilters, int aNumberOfWalls) {
...
void RobotWorldCanvas::handleLeftUp(MouseEvent &event) {
  RectangleShapePtr startRectangleShape =
      std::dynamic_pointer_cast<RectangleShape>(startActionShape);
  RectangleShapePtr endRectangeShape =
      std::dynamic_pointer_cast<RectangleShape>(
          getShapeAt(event.GetPosition()));
...
void RobotWorldCanvas::handleEndDrag(ShapePtr aShape) {
  ...
  WayPointShapePtr wayPointShape =
      std::dynamic_pointer_cast<WayPointShape>(aShape);
  if (wayPointShape) {
  ...
  RectangleShapePtr rectangleShape =
      std::dynamic_pointer_cast<RectangleShape>(aShape);
  if (rectangleShape) {
    for (ShapePtr shape : shapes) {
      WallShapePtr wall = std::dynamic_pointer_cast<WallShape>(shape);
      if (wall) {
...
void RobotWorldCanvas::handleAddRobot(CommandEvent &UNUSEDPARAM(event)) {
  RobotShapePtr robot(new RobotShape(
      Model::RobotWorld::getRobotWorld().newRobot("Robot", popupPoint)));
  addShape(robot);
  Refresh();
}
/**
...
void RobotWorldCanvas::handleEditRobot(CommandEvent &UNUSEDPARAM(event)) {
  RobotShapePtr shape =
      std::dynamic_pointer_cast<RobotShape>(getSelectedShape());
  if (shape) {
    std::string name =
        GetTextFromUser("The new Robot name:", "Enter a new name");
...
void RobotWorldCanvas::handleAddWayPoint(CommandEvent &UNUSEDPARAM(event)) {
  WayPointShapePtr wayPoint(new WayPointShape(
      Model::RobotWorld::getRobotWorld().newWayPoint("Joost", popupPoint)));
  addShape(wayPoint);
  Refresh();
}
/**
...
void RobotWorldCanvas::handleEditWayPoint(CommandEvent &UNUSEDPARAM(event)) {
  WayPointShapePtr shape =
      std::dynamic_pointer_cast<WayPointShape>(getSelectedShape());
  if (shape) {
    std::string name =
        GetTextFromUser("The new WayPoint name:", "Enter a new name");
...
void RobotWorldCanvas::handleAddGoal(CommandEvent &UNUSEDPARAM(event)) {
  GoalShapePtr goal(new GoalShape(
      Model::RobotWorld::getRobotWorld().newGoal("Goal", popupPoint)));
  addShape(goal);
  Refresh();
}
/**
...
void RobotWorldCanvas::handleEditGoal(CommandEvent &UNUSEDPARAM(event)) {
  ...
  if (shape) {
    std::string name =
        GetTextFromUser("The new Goal name:", "Enter a new name");
...
void RobotWorldCanvas::handleAddWall(CommandEvent &UNUSEDPARAM(event)) {
  ...
  ShapePtr wall(new WallShape(Model::RobotWorld::getRobotWorld().newWall(
                                  start->getCentre(), end->getCentre(), false),
                              start, end));
...
void RobotWorldCanvas::handleNotification(
  ...
  remove<Model::Path, View::PathShape>(
      Model::RobotWorld::getRobotWorld().getPaths());
  add<Model::Path, View::PathShape>(
      Model::RobotWorld::getRobotWorld().getPaths());
...
void RobotWorldCanvas::handleMenu(const Point &UNUSEDPARAM(aScreenPoint)) {
  ...
  Bind(
      wxEVT_COMMAND_MENU_SELECTED,
      [this](CommandEvent &anEvent) { this->OnAddGoal(anEvent); }, ID_ADD_GOAL);

  popupMenu.Append(ID_ADD_WALL, _T( "Add wall"), _T( "ID_ADD_WALL2"));
  Bind(
      wxEVT_COMMAND_MENU_SELECTED,
      [this](CommandEvent &anEvent) { this->OnAddWall(anEvent); }, ID_ADD_WALL);
 ...
 *
 */
void RobotWorldCanvas::OnPaint(PaintEvent &event) { handlePaint(event); }
/**
 *
 */
void RobotWorldCanvas::OnSize(SizeEvent &event) { handleSize(event); }
/**
 *
 */
void RobotWorldCanvas::OnLeftDown(MouseEvent &event) { handleLeftDown(event); }
/**
 *
 */
void RobotWorldCanvas::OnLeftUp(MouseEvent &event) { handleLeftUp(event); }
/**
 *
 */
 ...
 *
 */
void RobotWorldCanvas::OnMiddleUp(MouseEvent &event) { handleMiddleUp(event); }
/**
 *
 */
 ...
 *
 */
void RobotWorldCanvas::OnRightUp(MouseEvent &event) { handleRightUp(event); }
/**
 *
 */
 ...
 *
 */
void RobotWorldCanvas::OnKeyDown(KeyEvent &event) { handleKey(event); }
/**
 *
 */
void RobotWorldCanvas::OnCharDown(KeyEvent &event) { handleKey(event); }
/**
 *
 */
 ...
 *
 */
void RobotWorldCanvas::OnAddGoal(CommandEvent &event) { handleAddGoal(event); }
/**
 *
 */
 ...
 *
 */
void RobotWorldCanvas::OnAddWall(CommandEvent &event) { handleAddWall(event); }
/**
 *
 */
// cppcheck-suppress unusedFunction
