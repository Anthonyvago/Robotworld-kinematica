...
struct GMTOffset {
  GMTOffset() {
    ...
    auto local_time = boost::date_time::c_local_adjustor<
        boost::posix_time::ptime>::utc_to_local(utc_time);

    // Calculate the difference in milliseconds between local time and the UTC
    // time.
    auto time_t_diff = boost::posix_time::to_time_t(local_time) -
                       boost::posix_time::to_time_t(utc_time);
...
/* static */ ObjectId ObjectId::newObjectId() {
  ...
  // Prevent overflow to 0 within the second. This should only happen on fast
  // machines as the creation of the fraction part is processor speed dependent
  ...
  // We actually only use 27 chars but it could be 93, based on the maximum int
  // values....
  char timestampBuffer[93] = {'\0'};
  sprintf(timestampBuffer, "%04d-%02d-%02d-%02ld-%02d-%02d-%06lu",
          tm.tm_year + 1900, // @suppress("Avoid magic numbers")
          tm.tm_mon + 1, tm.tm_mday, tm.tm_hour + gmtOffset.offset, tm.tm_min,
          tm.tm_sec, fraction++);
 ...
 *
 */
/*virtual*/ std::string ObjectId::asDebugString() const { return asString(); }
