...
void MainFrameWindow::initialise() {
  ...
  Bind(
      ...
      [this](CommandEvent &anEvent) { this->OnQuit(anEvent); }, ID_QUIT);
  Bind(
      wxEVT_COMMAND_MENU_SELECTED,
  ...
  Bind(
      wxEVT_COMMAND_MENU_SELECTED,
      [this](CommandEvent &anEvent) { this->OnAbout(anEvent); }, ID_ABOUT);
...
MenuBar *MainFrameWindow::initialiseMenuBar() {
  ...
  debugMenu->AppendRadioItem(ID_WIDGET_TRACE_FUNCTION, WXSTRING("Widget"),
                             WXSTRING("Widget"));
  debugMenu->AppendRadioItem(ID_STDCOUT_TRACE_FUNCTION, WXSTRING("StdOut"),
                             WXSTRING("StdOut"));
  debugMenu->AppendRadioItem(ID_FILE_TRACE_FUNCTION, WXSTRING("File"),
                             WXSTRING("File"));
...
Panel *MainFrameWindow::initialiseRhsPanel() {
  if (!rhsPanel) {
    ...
    sizer->Add(logTextCtrl = new LogTextCtrl(rhsPanel, DEFAULT_ID,
...
Panel *MainFrameWindow::initialiseButtonPanel() {
  auto *panel = new Panel(rhsPanel, DEFAULT_ID, wxDefaultPosition,
  ...
  kalmanFilterToggle =
      makeToggleButton(panel, "Kalman filter", [this](CommandEvent &anEvent) {
  ...
  particleFilterToggle =
      makeToggleButton(panel, "Particle filter", [this](CommandEvent &anEvent) {
  ...
  sizer->Add(
      makeButton(panel, "Populate world",
                 [this](CommandEvent &anEvent) { this->OnPopulate(anEvent); }),
      1, wxALL | wxEXPAND | wxALIGN_CENTER_VERTICAL, 5);
  sizer->Add(makeButton(panel, "Clear world",
                        [this](CommandEvent &anEvent) {
                          this->OnUnpopulate(anEvent);
                        }),
             1, wxALL | wxEXPAND | wxALIGN_CENTER_VERTICAL, 5);
  sizer->Add(new wxStaticText(panel, DEFAULT_ID, WXSTRING("Compass:")), 1,
             wxALL | wxEXPAND | wxALIGN_CENTER_VERTICAL, 5);
  sizer->Add(compassSlider, 1, wxALL | wxEXPAND, 5);
  sizer->Add(makeButton(panel, "Reset compass",
                        [this](CommandEvent &anEvent) {
  ...
  sizer->Add(makeButton(panel, "Start robot",
             ...
             1, wxALL | wxEXPAND | wxALIGN_CENTER_VERTICAL, 5);
  sizer->Add(
      makeButton(panel, "Stop robot",
                 [this](CommandEvent &anEvent) { this->OnStopRobot(anEvent); }),
      1, wxALL | wxEXPAND | wxALIGN_CENTER_VERTICAL, 5);
  sizer->Add(new wxStaticText(panel, DEFAULT_ID, WXSTRING("Odometer:")), 1,
             wxALL | wxEXPAND | wxALIGN_CENTER_VERTICAL, 5);
  sizer->Add(odometerSlider, 1, wxALL | wxEXPAND, 5);
  sizer->Add(makeButton(panel, "Reset odometer",
                        [this](CommandEvent &anEvent) {
                          this->OnResetSliderOdometer(anEvent);
                        }),
             1, wxALL | wxEXPAND | wxALIGN_CENTER_VERTICAL, 5);
  sizer->Add(kalmanFilterToggle, 1, wxALL | wxEXPAND | wxALIGN_CENTER_VERTICAL,
             5);
  sizer->Add(particleFilterToggle, 1,
             wxALL | wxEXPAND | wxALIGN_CENTER_VERTICAL, 5);
  sizer->Add(new wxStaticText(panel, DEFAULT_ID, WXSTRING("Lidar:")), 1,
             wxALL | wxEXPAND | wxALIGN_CENTER_VERTICAL, 5);
  sizer->Add(lidarSlider, 1, wxALL | wxEXPAND, 5);
  sizer->Add(makeButton(panel, "Reset lidar",
                        [this](CommandEvent &anEvent) {
...
void MainFrameWindow::OnPopulate(CommandEvent &UNUSEDPARAM(anEvent)) {
  ...
  std::vector<Model::eFilterType> filters;

  if (kalmanFilterToggle->GetValue()) {
    filters.emplace_back(Model::eFilterType::KALMAN_FILTER);
  }
  if (particleFilterToggle->GetValue()) {
    filters.emplace_back(Model::eFilterType::PARTICLE_FILTER);
  }
...
void MainFrameWindow::OnKalmanToggle(CommandEvent &UNUSEDPARAM(anEvent)) {
  ...
  if (robot) {
    if (kalmanFilterToggle->GetValue()) {
      ...
      robot->addFilter(Model::eFilterType::KALMAN_FILTER);
      kalmanFilterToggle->SetBackgroundColour(KALMAN_BLUE);
    } else {
      LOG("removing kalman filter from robot");
      robot->removeFilter(Model::eFilterType::KALMAN_FILTER);
...
void MainFrameWindow::OnParticleToggle(CommandEvent &UNUSEDPARAM(anEvent)) {
  ...
  if (robot) {
    if (particleFilterToggle->GetValue()) {
      ...
      robot->addFilter(Model::eFilterType::PARTICLE_FILTER);
      particleFilterToggle->SetBackgroundColour(PARTICLE_GREEN);
    } else {
      LOG("removing particle filter from robot");
      robot->removeFilter(Model::eFilterType::PARTICLE_FILTER);
...
void MainFrameWindow::OnResetSliderOdometer(
    CommandEvent &UNUSEDPARAM(anEvent)) {
  odometerSlider->SetValue(static_cast<int32_t>(odometerConfigValue));
