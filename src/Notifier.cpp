...
void Notifier::enableNotification(bool enable /* = true */) { notify = enable; }
/**
 *
 */
void Notifier::disableNotification() { notify = false; }
/**
 *
 */
// cppcheck-suppress unusedFunction
bool Notifier::isEnabledForNotification() const { return notify; }
/**
 *	The implementation of operator== uses pointer comparison!
 */
void Notifier::addObserver(Observer &anObserver) {
  if (std::find_if(observers.begin(), observers.end(),
                   [&anObserver](const Observer *observer) {
 ...
 *
 */
std::string Notifier::asDebugString() const { return asString(); }
} // namespace Base
