 ...
 *
 */
LineShape::LineShape(RectangleShapePtr aNode1, RectangleShapePtr aNode2,
                     const std::string &aTitle /*= ""*/, int aLineWidth /*= 1*/,
                     int anArrowHeadSize /*=10*/)
    : node1(aNode1), node2(aNode2), title(aTitle), lineWidth(aLineWidth),
      arrowHeadSize(anArrowHeadSize), top(), right(), left() {}
/**
 *
 */
LineShape::LineShape(Model::ModelObjectPtr aModelObject,
                     RectangleShapePtr aNode1, RectangleShapePtr aNode2,
                     const std::string &aTitle /*= ""*/, int aLineWidth /*= 1*/,
                     int anArrowHeadSize /*= 10*/)
    : Shape(aModelObject), node1(aNode1), node2(aNode2), title(aTitle),
      lineWidth(aLineWidth), arrowHeadSize(anArrowHeadSize), top(), right(),
      left() {}
/**
 *
 */
...
void LineShape::draw(wxDC &dc) {
  ...
  double angle =
      Utils::Shape2DUtils::getAngle(node1->getCentre(), node2->getCentre());
...
void LineShape::drawHead(wxDC &dc) {
  ...
  // Second we rotate the triangle with the angle the line makes with the Y-axis
  // around it's centre Than we move the centre of the triangle to the end of
  // the line, but outside the node
  ...
  Point triangleCentre(static_cast<int>(getBegin().x + dX),
                       static_cast<int>(getBegin().y - dY));
  ...
  dc.DrawCircle(top, 2);
  dc.SetPen(wxPen(WXSTRING("GREEN"), 2,
                  wxPENSTYLE_SOLID)); // stuuRRRRRRboord RRRRRRechts gRRRRRRoen
 ...
 *
 */
std::string LineShape::getTitle() const { return title; }
/**
 *
 */
void LineShape::setTitle(const std::string &aTitle) { title = aTitle; }
/**
 *
 */
int LineShape::getLineWidth() const { return lineWidth; }
/**
 *
 */
// cppcheck-suppress unusedFunction
void LineShape::setLineWidth(int aLineWidth) { lineWidth = aLineWidth; }
/**
 *
 */
// cppcheck-suppress unusedFunction
int LineShape::getArrowHeadSize() const { return arrowHeadSize; }
/**
 *
 */
// cppcheck-suppress unusedFunction
 ...
 *
 */
Point LineShape::getBegin() const { return node1->getCentre(); }

/**
 *
 */
Point LineShape::getEnd() const { return node2->getCentre(); }
...
std::string LineShape::asString() const {
  ...
  os << Shape::asString() << " LineShape: name = " << title << " "
     << Utils::Shape2DUtils::asString(getBegin()) << "->"
