...
template <typename T, const std::size_t M /* number of rows */,
...
public:
  ...
  /**
   * Ctor with a list of lists of values where aList must contain M elements and
   * each list in aList must contain N elements
   */
   ...
   *
   */
  inline std::size_t getRows() const { return M; }
  /**
   *
   */
  inline std::size_t getColumns() const { return N; }
  //@}
  /**
   * @name Element access
   */
   ...
   * If aRowIndex > getRows() or aColumnIndex > getColumns an exception of type
   ...
   *
   */
  template <class T2 = T> Matrix<T, M, N> &operator*=(const T2 &scalar);
  /**
   *
   */
  template <class T2 = T> Matrix<T, M, N> operator*(const T2 &scalar) const;
  /**
   *
   */
  template <class T2 = T> Matrix<T, M, N> &operator/=(const T2 &scalar);
...
/**
 * Compare two row vectors using a aPrecision and a factor. The actual used
 * precision is aPrecission*aFactor.
 *
 */
...
/**
 * Compare two column vectors using a aPrecision and a factor. The actual used
 * precision is aPrecission*aFactor.
 */
