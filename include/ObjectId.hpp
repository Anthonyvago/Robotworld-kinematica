 ...
 *
 * The OID class is 16 bytes, this class is 24 bytes. If one is memory
 * constraint the OID class may be of some help. Be warned: the OID class is not
 * actively maintained anymore (but than, who reeds this besides me, the
 * maintainer).
 */
...
public:
  ...
  /**
   * If an ObjectId should be *really* universal unique every application should
   * have its own namespace
   */
  static std::string objectIdNamespace;

  /**
   * This function returns an ObjectId that is guaranteed to be unique in the
   * application it is generated in. If multiple application use the same
   * library it is the responsibility of the applications to ensure uniqueness
   ...
   * Data ctor.
   *
   * @param max 		Initial elements reservation. Is actually
   * ignored
   * @param length 	Length of the data
   * @param data 		Start of the data
   * @param release 	If true the sequences owns and manages any containing
   * pointers Is actually ignored and always true.
   */
  ObjectId(unsigned long /*max*/, unsigned long len,
           unsigned char *aDataPointer, bool /*rel = true-*/)
  ...
  virtual ~ObjectId();
  /**
   * @return The virtual maximum of bytes for this ObjectId. This maximum can
   * safely be ignored.
   *
   */
  unsigned long maximum() const { return base::capacity(); }

  /**
   * If the current length of the sequence is less than the new length we append
   * new elements which are created by calling the default constructor.  If the
   * current length is more than the new length we just cut of the tail of the
   * sequence.
   *
   ...
   */
  unsigned long length() const { return base::size(); }
   ...
   */
  reference operator[](unsigned long index) { return base::operator[](index); }
   ...
   */
  void replace(unsigned long /*max*/, unsigned long len, pointer newData,
               bool /*rel = true*/) {
   ...
   * 			If the buffer is modified as that it expands the buffer this
   * \will lead to failure eventually
   */
   ...
   */
  const_pointer get_buffer() const { return &base::operator[](0); }
   ...
   */
  static void freebuf(pointer aT) { delete[] aT; }
  ...
  bool operator<(const ObjectId &anObjectId) const;
  /**
   * Calling obj1.fromString( obj2.toString()) has the same effect as
