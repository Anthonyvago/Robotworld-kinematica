...
public:
   ...
   *
   */
  Client(const std::string &aHostName, unsigned short aPort,
  ...
  void dispatchMessage(const Message &aMessage) {
    ...
    boost::asio::ip::tcp::resolver resolver(
        CommunicationService::getCommunicationService().getIOContext());
    ...
    session->getSocket().async_connect(
        endpoint, // @suppress("Method cannot be resolved")
...
private:
  ...
  void handleConnect(ClientSession *aSession,
    ...
    if (!error) {
      ...
      os << __PRETTY_FUNCTION__ << ": error connecting to " << host
         << " at port " << port << ", reason: " << error.message();
