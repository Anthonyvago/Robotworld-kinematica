...
struct Message {
  ...
  struct MessageHeader {
    ...
    /**
     * The ASCII representation is suitable for parsing by
     * MessageHeader::fromString.
     *
    ...
    std::string toString() const {
      ...
      os << magicNumber1 << magicNumber2 << magicNumber3 << magicNumber4
         << majorVersion << minorVersion << std::setw(charWidth)
         << static_cast<int>(messageType) << std::setw(intWidth)
         << messageLength;
      return os.str();
    }
    /**
    ...
    void fromString(const std::string &aString) {
      ...
      is >> magic[0] >> magic[1] >> magic[2] >> magic[3] >> major >> minor >>
          std::setw(charWidth) >> reinterpret_cast<int &>(messageType) >>
          std::setw(intWidth) >> messageLength;
    }
    /**
    ...
    /**
     *
     * @return The message type which should be one of the types in
     * MessageTypes.hpp
     */
    char getMessageType() const { return messageType; }
    /**
     * @return The length of the message in bytes
     */
    unsigned long getMessageLength() const { return messageLength; }
    /**
    ...
    std::string asString() const {
      ...
      os << magicNumber1 << magicNumber2 << magicNumber3 << magicNumber4 << " "
         ...
         << static_cast<int>(messageType) << " " << messageLength;
      return os.str();
    }
    /**
     *
     */
    std::string asDebugString() const { return asString(); }
    //@}
  ...
  /**
   *
   * @param aMessageType, should be one of the values mentioned in
   * MessageTypes.hpp
   * @param aMessage, which is in a ordinary OO-RPC a specification of the
   * object, the function and the arguments to call.
   */
   ...
   * @param aMessageType
   */
  void setMessageType(char aMessageType) { messageType = aMessageType; }
  /**
   *
   * @return
   */
  MessageBody getBody() const { return message; }
  /**
   *
   * @param aBody
   */
  void setBody(const std::string &aBody) { message = aBody; }
  /**
   * @return The length of the message in bytes
   */
  unsigned long length() const { return message.length(); }
  /**
   * @name Debug functions
   */
  //@{
   ...
   * debugging
   */
  virtual std::string asDebugString() const { return asDebugString(); }
  //@}
  /**
   *
   */
