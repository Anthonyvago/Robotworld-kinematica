...
/**
 * The CommandlineArgument class encapsulates any command line argument given to
 * the application
 */
...
public:
  ...
  //@{
  CommandlineArgument()
      : argumentNumber(std::numeric_limits<unsigned long>::max()), variable(),
        value() {}
  /**
   *
  ...
  CommandlineArgument(unsigned long anArgumentNumber,
                      const std::string &aVariable, const std::string &aValue)
   ...
   */
  CommandlineArgument &
  operator=(const CommandlineArgument &aCommandlineArgument) {
