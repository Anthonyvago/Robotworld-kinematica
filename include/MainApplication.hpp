...
public:
   ...
   * initialisation: doing it here and not in the ctor allows to have an error
   * return
   *
   ...
   * 1. Any argument starting with "-" that has "=" in it somewhere is treated
   * as "argument = value". Spaces are not allowed.
   * 2. Any argument starting with a "-" that has no "=" in it somewhere is
   * treated as a boolean with the value "true". There are no variables that can
   * be false.
   ...
   * 					Be aware that "-" is NOT stripped from the
   * argument. The comparisson is done by operator==( const string&).
