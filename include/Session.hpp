...
public:
  ...
  /**
   * Handle the fact that a message is read. This function is called by the
   * framework after the message (header + body) is read. Normally this is the
   * only function that a ServerSession or ClientSession has to implement.
   *
   * If message.getMessageType is CommunicationReadError the reading of the
   * message failed so an appropriate action should be taken.
   */
  virtual void handleMessageRead() = 0;
  /**
   * Handle the fact that a message is written. This function is called by the
   * framework after the message (header + body) is written. Normally this is
   * the only function that a ServerSession or ClientSession has to implement.
   *
   * If message.getMessageType is CommunicationWriteError the writing of the
   * message failed so an appropriate action should be taken.
   */
  virtual void handleMessageWritten() = 0;
  /**
   * This function must be public or Client and Server should be friend of
   * Session
   *
...
protected:
  /**
   * readMessage will read the message in 2 a-sync reads, 1 for the header and 1
   * for the body. After each read a callback will be called that should handle
   * the bytes just read. After reading the full message handleMessageRead will
   * be called whose responsibility it is to handle the message as a whole.
   *
  ...
  void handleHeaderRead(const boost::system::error_code &error,
    ...
    if (!error) {
      message.setHeader(Message::MessageHeader(std::string(
          headerBuffer.begin(),
          headerBuffer.end()))); // @suppress("Symbol is not resolved")
  ...
  void handleBodyRead(const boost::system::error_code &error,
    ...
    if (!error) {
      message.setBody(
          std::string(bodyBuffer.begin(),
                      bodyBuffer.end())); // @suppress("Symbol is not resolved")
  ...
  /**
   * writeMessage will write the message in 2 a-sync writes, 1 for the header
   * and 1 for the body. After each write a callback will be called that should
   * handle the bytes just read. After writing the full message
   * handleMessageWritten will be called.
   *
  ...
  void writeMessage(const Message &aMessage) {
    ...
    boost::asio::async_write(
        ...
        boost::asio::buffer(message.getHeader().toString(),
                            message.getHeader().getHeaderLength()),
...
public:
   ...
   * @see Session::start()
   */
  virtual void start() override { readMessage(); }
  /**
  ...
  virtual void handleMessageRead() override {
    if (message.getMessageType() != CommunicationReadError) {
      ...
      TRACE_DEVELOP("*** ServerSession::handleMessageRead: " +
                    message.asString());
      delete this;
    }
  }
  ...
  virtual void handleMessageWritten() override {
    if (message.getMessageType() != CommunicationWriteError) {
      ...
      TRACE_DEVELOP("*** ServerSession::handleMessageWritten: " +
                    message.asString());
    }
...
public:
   ...
   * @see Session::start()
   */
  virtual void start() override { writeMessage(message); }
  /**
   * @see Session::handleMessageRead( Message& aMessage)
   */
  virtual void handleMessageRead() override {
    if (message.getMessageType() != CommunicationReadError) {
      ...
      TRACE_DEVELOP("*** ClientSession::handleMessageRead: " +
                    message.asString());
    }
  ...
  virtual void handleMessageWritten() override {
    if (message.getMessageType() != CommunicationWriteError) {
      ...
      TRACE_DEVELOP("*** ClientSession::handleMessageWritten: " +
                    message.asString());
      delete this;
    }
  }
