...
public:
  ...
  /**
   * The canvas is partially displayed in a scrolling window. Hence, the user
   * viewable part of this window is probably just a part of this window. This
   * function takes a point in the (visible) scrolling window coordinates and
   * translates it to this canvas windows coordinates. It takes into account the
   * number of lines and number of pixels between lines.
   */
  Point devicePointFor(const Point &aScreenPoint) const;
  /**
   * This function is the opposite of devicePointFor(): it translates canvas
   * coordinates to scrolling window coordinates
   *
   ...
   * If selectionEnabled is set to true, handleSelection() of both the old and
   * new selected shape will be called after setting their selection as
   * appropriate.
   *
   * @param 	aSelectedShape 	The new Shape that will be selected. If a
   * nullptr is passed  it acts as a mere de-selection of the current selected
   * shape
   */
   ...
   *
   * @param 	aPoint
   * @return 	True if any Shape returns true for Shape.ocuppies(aPoint), false
   * otherwise.
   *
  ...
  /**
   *
   * @param 	aPoint A screen point, i.e. a point on the screen, not on the
   * (scrollable) canvas.
   * @return 	The first Shape in iteration order that returns true for
   * Shape.ocuppies(aPoint). If no such Shape exists nullptr will be returned.
   */
   ...
   * filter any events it is interested in. This function should only be called
   * from the main thread because wxWidgets does not allow for painting widgets
   * in a background thread: all painting should be done in the main thread.
   * Therefore all notifications are routed to handleBackGroundNotification() as
   * a convenience.
   *
   */
  ...
  //@}
  /**
   * A Notifier that runs in a background thread should call this function
   * instead of handleNotification(). handleNotification() is routed to this
   * function as a convenience. Bad for performance though.
   */
  virtual void handleBackGroundNotification();
  /**
   * Asks the world to populates itself with a robot, a goal and the given
   * number of walls
   */
  void populate(const std::vector<Model::eFilterType> &activeFilters,
                int aNumberOfWalls = 2);
  /**
...
private:
  ...
  Base::NotificationHandler<std::function<void(NotifyEvent &)>>
      *notificationHandler;

  /**
   * This function removes all Shapes that look at a ModelObject that is not
   * longer in RobotWorld
   */
  template <typename T, typename S>
  void remove(const std::vector<std::shared_ptr<T>> &aTs) {
    std::vector<ShapePtr> shapePtrs;
    std::copy_if(
        shapes.begin(), shapes.end(), std::back_inserter(shapePtrs),
        [&aTs](ShapePtr aShape) {
          std::shared_ptr<S> shape = std::dynamic_pointer_cast<S>(aShape);
          if (shape) {
            return std::find_if(
                       aTs.begin(), aTs.end(), [shape](std::shared_ptr<T> aT) {
                         return shape->getModelObject()->getObjectId() ==
                                aT->getObjectId();
                       }) == aTs.end();
          }
          return false;
  ...
  /**
   * This function add Shapes for ModelObjects that are in RobotWorld but that
   * have no Shape yet
   */
  template <typename T, typename S>
  void add(const std::vector<std::shared_ptr<T>> &aTs) {
    for (std::shared_ptr<T> t : aTs) {
      auto result =
          std::find_if(shapes.begin(), shapes.end(), [t](ShapePtr aShape) {
            if (aShape->getModelObject()) {
              return aShape->getModelObject()->getObjectId() ==
                     t->getObjectId();
            }
            return false;
          });
